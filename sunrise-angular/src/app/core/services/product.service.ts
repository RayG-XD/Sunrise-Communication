...
export class ProductService {
  ...
  // Performance Optimization: Cache product lookup Map to avoid O(N) array search on slug lookup
  readonly productsBySlugMap = computed(() => {
  ...
  /**
   * Returns a computed signal finding a product by its URL slug in O(1) time.
   */
  getProductBySlug(slug: string): Signal<Product | undefined> {
    return computed(() => {
      return this.productsBySlugMap().get(slug);
    });
  }
}
