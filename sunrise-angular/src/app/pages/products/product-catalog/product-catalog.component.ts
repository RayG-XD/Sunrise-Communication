...
export class ProductCatalogComponent implements OnInit {
  ...
  // Performance Optimization: Short-circuit filtering using fast O(1) Set checks for category,
  // sub-category, and brand prior to running expensive multi-field string lowercasing & search matches.
  filteredProducts = computed(() => {
    ...
    return products.filter((p) => {
      // Fast O(1) Set checks first to avoid redundant string allocations & searching for non-matching items
      ...
      // Multi-field search match only evaluated for products that pass category/brand filters
      return (
